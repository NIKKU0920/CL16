...
def parse_args():
    ...
    parser.add_argument('--alpha', type=float, default=0.1, help='Value for mixup and RICAP')
...
def main(args):
    ...
    for epoch in range(initial_epoch, total_epochs + 1):
        ...
        if num_completed_sets > 0 and num_completed_sets > num_completed_sets_prev:
            ...
            add_values = [0.032, 0.041, 0.055, 0.072, 0]  # Add certain value every 40 epochs
            ...
            args.alpha += add_values[add_index]
            value_to_add = add_values[add_index]
            train_loader = data_config_CL(args)
            print(f"Epoch {epoch}: Added {value_to_add} to args.alpha to New value: {args.alpha}")
